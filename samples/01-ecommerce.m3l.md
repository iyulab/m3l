...
- email: email @unique "Primary contact email"
...
- total_orders: integer @computed("COUNT(Order.id)")
- total_spent: decimal(12,2) @computed("SUM(Order.grand_total)")
- avg_order_value: decimal(10,2) @computed("AVG(Order.grand_total)")
- last_order_date: timestamp @computed("MAX(Order.ordered_at)")
